...
public class Jdk7ConcurrentSkipListMap<K,V> extends AbstractMap<K,V>
     ...
     */
    public Jdk7ConcurrentSkipListMap() {
     ...
     */
    public Jdk7ConcurrentSkipListMap(Comparator<? super K> comparator) {
     ...
     *         or values are null
     */
    public Jdk7ConcurrentSkipListMap(Map<? extends K, ? extends V> m) {
        this.comparator = null;
        initialize();
        putAll(m);
    }
     ...
     */
    public Jdk7ConcurrentSkipListMap(SortedMap<K, ? extends V> m) {
     ...
     */
    public Jdk7ConcurrentSkipListMap<K,V> clone() {
        Jdk7ConcurrentSkipListMap<K,V> clone = null;
        try {
            clone = (Jdk7ConcurrentSkipListMap<K,V>) super.clone();
    ...
    abstract class Iter<T> implements Iterator<T> {
        ...
        public void remove() {
            ...
            Jdk7ConcurrentSkipListMap.this.remove(l.key);
            lastReturned = null;
        }
    ...
    static final class Values<E> extends AbstractCollection<E> {
        ...
        public Iterator<E> iterator() {
            if (m instanceof Jdk7ConcurrentSkipListMap)
                return ((Jdk7ConcurrentSkipListMap<Object,E>)m).valueIterator();
            else
    ...
    static final class EntrySet<K1,V1> extends AbstractSet<Map.Entry<K1,V1>> {
        ...
        public Iterator<Map.Entry<K1,V1>> iterator() {
            if (m instanceof Jdk7ConcurrentSkipListMap)
                return ((Jdk7ConcurrentSkipListMap<K1,V1>)m).entryIterator();
            else
    ...
    static final class SubMap<K,V> extends AbstractMap<K,V>
        ...
        /** Underlying map */
        private final Jdk7ConcurrentSkipListMap<K,V> m;
         ...
         */
        SubMap(Jdk7ConcurrentSkipListMap<K,V> map,
         ...
         */
        private boolean isBeforeEnd(Jdk7ConcurrentSkipListMap.Node<K,V> n) {
            if (n == null)
                return false;
            if (hi == null)
                return true;
         ...
         */
        private Jdk7ConcurrentSkipListMap.Node<K,V> loNode() {
            if (lo == null)
                return m.findFirst();
            else if (loInclusive)
         ...
         * most usages need to check bounds
         */
        private Jdk7ConcurrentSkipListMap.Node<K,V> hiNode() {
            if (hi == null)
                return m.findLast();
            else if (hiInclusive)
         ...
         */
        private K lowestKey() {
            Jdk7ConcurrentSkipListMap.Node<K,V> n = loNode();
            if (isBeforeEnd(n))
                return n.key;
            else
                throw new NoSuchElementException();
        ...
        private K highestKey() {
            Jdk7ConcurrentSkipListMap.Node<K,V> n = hiNode();
            if (n != null) {
                K last = n.key;
                if (inBounds(last))
                    return last;
        ...
        public int size() {
            long count = 0;
            for (Jdk7ConcurrentSkipListMap.Node<K,V> n = loNode();
                 isBeforeEnd(n);
                 n = n.next) {
                if (n.getValidValue() != null)
                    ++count;
